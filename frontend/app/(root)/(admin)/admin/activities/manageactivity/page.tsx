...
const ManageActivity = () => {
    ...
    useEffect(() => {
        if (id) {
            ...
            // Assuming fetchActivity is a function to fetch activity details
    ...
    const fetchActivity = async (id: number) => {
        try {
            const response = await fetch(`${process.env.ADMINURL}/api/getactivity?id=${id}`);
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const response = await fetch(`${process.env.ADMINURL}/api/addactivity`, {
                method: 'POST',
                body: formData,
            });

            if (response.ok) {
                message.success(`${id ? 'Activity updated successfully' : 'Activity added successfully'}`);
                // Optionally, you can perform further actions after successful submission
                router.push('/admin/activities/all');
            } else {
    ...
    return (
        <div className="bg-white p-4">
            <PageHeaderWithBreadcrumb
                crumbs={[{ title: 'Home', href: '/admin' }, { title: 'Activities', href: '/admin/activities/all' }, { title: `${id ? 'Edit' : 'Create'} Activity` }]}
                title={`${id ? 'Edit' : 'Create'} Activity`}
                description=""
            />
